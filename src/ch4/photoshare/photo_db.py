import re
import photo_file, photo_sqlite
...
def album_new(user_id, args):
    ...
    if name == '':
        return 0
    album_id = exec('INSERT INTO albums (name, user_id) VALUES (?, ?)',
                    name, user_id)
    return album_id
...
def save_file(user_id, upfile, album_id):
    if not re.search(r'\.(jpg|jepg)$', upfile.filename):
    ...
    file_id = exec('INSERT INTO files (user_id, filename, album_id) VALUES (?, ?, ?)',
...
def get_album_files(album_id):
    return select('SELECT * FROM files WHERE album_id=? ORDER BY file_id DESC',
                  album_id)

def get_user_files(user_id):
    a = select('SELECT * FROM files WHERE user_id=? ORDER BY file_id DESC LIMIT 50',
               user_id)
    for i in a:
        i['name'] = get_album_name(i['album_id'])
    return a
